...
from tests.conftest import MockGlassFlowClient, MockVLClient, MockVMClient, make_registry
...
def _make_server(gf=None, vm=None, vl=None):
    ...
    reg = make_registry(gf, vm, vl)
    return create_server(reg, port=0), gf, vm, vl
...
class TestGetDlqState:
    ...
    def test_not_found(self):
        ...
        assert "Error" in _get_tool(mcp, "get_dlq_state")("missing")
...
class TestQueryPipelineMetrics:
    def test_with_data(self):
        ...
        vm.set_results([{"metric": {"pipeline_id": "metric-pipe"}, "value": [1234567, "42.5"]}])
        result = json.loads(
    ...
    def test_unknown_metric(self):
        mcp, _, _, _ = _make_server()
        assert "Unknown metric" in _get_tool(mcp, "query_pipeline_metrics")("pipe", "nonexistent")

    def test_invalid_pipeline_id(self):
        mcp, _, _, _ = _make_server()
        assert "Invalid pipeline_id" in _get_tool(mcp, "query_pipeline_metrics")(
...
class TestQueryPipelineLogs:
    ...
    def test_invalid_component_rejected(self):
        mcp, _, _, _ = _make_server()
        assert "Invalid component" in _get_tool(mcp, "query_pipeline_logs")(
            "pipe", component='sink" OR _msg:*'
        )
...
class TestDiagnosePipeline:
    ...
    def test_invalid_pipeline_id(self):
        mcp, _, _, _ = _make_server()
        assert "Invalid pipeline_id" in _get_tool(mcp, "diagnose_pipeline")('bad"; id')
